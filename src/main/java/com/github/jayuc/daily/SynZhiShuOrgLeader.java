...
public class SynZhiShuOrgLeader {
    @ToString
    static class User{
        String name;
        int id;
        ...
        String position;
        String toSql(){
            return "update t_sys_org set leader_id = " + id + " where org_id = " + orgId + position;
        }
    }
    public static void main(String[] args) {
        ...
        String sql1 = "select a.user_id,a.user_name,a.org_id,a.position from t_sys_user a inner join t_sys_org b on a.org_id = b.org_id " +
                "where b.leader_id = 0 and b.yes_office = 0";
...
//                    break;
//                }
//                if(user.position.equals("局长、经理、党组书记")){
//                    sqls.add(user.toSql());
//                    hasLeader = true;
//                    break;
//                }
                if(user.position.indexOf("局长") > -1){
                    sqls.add(user.toSql());
                    hasLeader = true;
                    break;
                }
